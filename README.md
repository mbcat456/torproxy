...
Each circuit is checked against `checkip.amazonaws.com` after construction. Duplicate public exit IPs are recorded and preferred for replacement, but working circuits are not discarded only because another circuit shares their egress IP. This preserves pool capacity while keeping the observed unique-exit count visible.

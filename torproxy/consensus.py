...
class RelayInfo:
    ...
    def is_usable(self) -> bool:
        return self.is_running() and self.is_valid() and self.has_ntor_key()

    def can_guard(self) -> bool:
        return self.is_guard() and self.is_usable()

    def can_exit(self) -> bool:
        return self.is_exit() and self.is_usable()


def parse_consensus(path: str) -> list[RelayInfo]:
    relays: list[RelayInfo] = []
    current: RelayInfo | None = None
            ...
            if line.startswith("r "):
                ...
                if len(parts) >= 8:
                    ...
                    try:
                        ...
                        decoded = base64.b64decode(ident)
                        current.identity = (
                            decoded if len(decoded) == 20 else b"\x00" * 20
                        )
                    except Exception:
...
def parse_microdescs(path: str) -> dict[str, bytes]:
    ...
    markers = [
        m.start() for m in re.finditer(rb"(?m)^(?:@last-listed\s|onion-key\s*$)", raw)
    ]
    starts = [start for start in markers if raw.startswith(b"onion-key", start)]
    result: dict[str, bytes] = {}

    for start in starts:
        next_idx = bisect.bisect_right(markers, start)
        end = markers[next_idx] if next_idx < len(markers) else len(raw)
        body = raw[start:end]
        if not body:
            continue
        ...
        for line in body.split(b"\n"):
            if line.startswith(b"ntor-onion-key "):
                ...
                try:
                    key_bytes = base64.b64decode(key_b64 + "=" * (-len(key_b64) % 4))
                    if len(key_bytes) == 32:
...
def load_relays(consensus_path: str, microdescs_path: str) -> list[RelayInfo]:
    ...
    usable = [r for r in relays if r.is_usable()]
...
def fetch_fresh_consensus(
    cache_consensus: str, cache_microdescs: str, progress_callback=None
) -> bool:
    ...
    BATCH_SIZE = 90
    batches = [
        md_hashes[i : i + BATCH_SIZE] for i in range(0, len(md_hashes), BATCH_SIZE)
    ]
    ...
    for batch_idx, batch in enumerate(batches):
        ...
        for attempt in range(3):
            try:
                ...
                log.debug(
                    "Batch %d attempt %d failed: %s", batch_idx + 1, attempt + 1, e
                )
                time.sleep(2)

        if data is None:
            log.warning(
                "Batch %d/%d failed after 3 attempts", batch_idx + 1, len(batches)
            )
            continue

        starts = [m.start() for m in re.finditer(rb"(?m)^onion-key\s*$", data)]
        if not starts:
            log.warning(
                "Batch %d returned no recognizable microdescriptors", batch_idx + 1
            )
            continue
        for part_idx, start in enumerate(starts):
            end = starts[part_idx + 1] if part_idx + 1 < len(starts) else len(data)
            body = data[start:end]
            if not body.strip():
                continue
            output.extend(b"@last-listed " + fake_date + b"\n")
            output.extend(body)
    ...
    md_data = bytes(output)
    microdescs_dir = os.path.dirname(cache_microdescs) or "."
    consensus_dir = os.path.dirname(cache_consensus) or "."
    os.makedirs(microdescs_dir, exist_ok=True)
    os.makedirs(consensus_dir, exist_ok=True)

    consensus_tmp = cache_consensus + ".tmp"
    microdescs_tmp = cache_microdescs + ".tmp"
    with open(consensus_tmp, "wb") as f:
        f.write(consensus_data)
    with open(microdescs_tmp, "wb") as f:
        f.write(md_data)
    os.replace(consensus_tmp, cache_consensus)
    os.replace(microdescs_tmp, cache_microdescs)

    entries = md_data.count(b"@last-listed ")
    log.info(
        "Saved %d KB microdescriptors (%d entries, %d batches)",
        len(md_data) // 1024,
        entries,
        len(batches),
    )
